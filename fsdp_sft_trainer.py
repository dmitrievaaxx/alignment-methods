...
os.environ['WANDB_DIR'] = '/job/output/wandb'
...
# ✅ Остальные импорты (verl и utils)
...
#             "Please install it by running: pip install flash-attn>=2.5.0\n"
#             "Or ensure requirements.txt is installed before running the script."
#         )
# elif is_npu_available:
#     from transformers.integrations.npu_flash_attention import index_first_axis, pad_input, rearrange, unpad_input
# else:
#     # When CUDA/NPU is not available (e.g., during DataSphere module discovery on local machine),
#     # we create stub functions that will raise an error when actually called during training.
#     # This allows the module to be imported without errors, but ensures training fails with
#     # a clear message if attempted without CUDA/NPU.
#     def _raise_cuda_required(*args, **kwargs):
#         raise RuntimeError(
#             "CUDA or NPU is required for this trainer. "
#             "This trainer should be run on a server with GPU support."
#         )
#     index_first_axis = _raise_cuda_required
#     pad_input = _raise_cuda_required
#     rearrange = _raise_cuda_required
#     unpad_input = _raise_cuda_required
...
class FSDPSFTTrainer:
    ...
    #         # dump_threads()  # ← ДАМП ПЕРЕД ОЧИСТКОЙ
        
    #     # 1. Close wandb explicitly
    #     try:
    #         import wandb
    ...
    #     except Exception as e:
    #         if rank == 0:
    #             print(f"⚠️ WandB close warning: {e}")
    ...
    #         if task:
    #             if rank == 0:
    #                 print("🔒 Closing ClearML task...")
    #             task.close()
    #             if rank == 0:
    #                 print("✅ ClearML closed")
    #     except Exception as e:
    #         if rank == 0:
    #             print(f"⚠️ ClearML close warning: {e}")
    ...
    #     if rank == 0:
    #         print("🔍 Debug: After closing trackers")
    #         # dump_threads()  # ← ДАМП ПОСЛЕ ЗАКРЫТИЯ ТРЕКЕРОВ
        
    #     # 4. Force destroy process group
    #     try:
    #         if rank == 0:
    ...
    #     except Exception as e:
    #         if rank == 0:
    #             print(f"⚠️ Process group destroy warning: {e}")
        
    #     if rank == 0:
    #         print("🔍 Debug: Final dump before exit")
    #         # dump_threads()  # ← ФИНАЛЬНЫЙ ДАМП ПЕРЕД ВЫХОДОМ
    ...
    def _cleanup(self):
        ...
        # 1. Даем время для завершения операций записи
        import time
        time.sleep(5)
        
        # 2. Закрываем трекеры в правильном порядке
        try:
            # Сначала ClearML
            from clearml import Task
            task = Task.current_task()
            if task:
                if rank == 0:
                    print("🔒 Closing ClearML...")
                task.flush(wait_for_uploads=True)  # ⭐ Ждем загрузку файлов
                task.close()
        except Exception as e:
            if rank == 0:
                print(f"⚠️ ClearML close: {e}")
        ...
        try:
            ...
            if rank == 0:
                print(f"⚠️ WandB close: {e}")
        
        # 3. Явная синхронизация файловой системы
        if rank == 0:
            import subprocess
            subprocess.run(["sync"], check=True)
            time.sleep(3)
        
        # 4. Убеждаемся, что все процессы завершили запись
        if torch.distributed.is_initialized():
            torch.distributed.barrier()
        
        # 5. Нормальное завершение вместо принудительного
        if rank == 0:
            print("✅ Cleanup completed")

    def _verify_output_files(self):
        """Проверяем создание выходных файлов"""
        import glob
        print("📁 Checking output directory structure:")
        
        # Проверяем W&B файлы
        wandb_files = glob.glob("/job/output/wandb/**/*", recursive=True)
        print(f"📊 W&B files: {len(wandb_files)}")
        for f in wandb_files[:5]:  # Покажем первые 5 файлов
            print(f"  - {f}")
        
        # Проверяем чекпоинты
        checkpoint_files = glob.glob("/job/output/checkpoints/**/*", recursive=True)
        print(f"💾 Checkpoint files: {len(checkpoint_files)}")
        
        # Создаем маркер завершения
        with open("/job/output/training_completed.txt", "w") as f:
            f.write(f"Training completed at step {self.global_step}\n")
    ...
    def training_step(self, batch: TensorDict):
        ...
        if self.global_step >= self.config.trainer.max_steps:
            if self.device_mesh.get_rank() == 0:
                print(f"⏹️ Skipping training step {self.global_step}, max_steps reached")
            return {"train/loss": 0, "train/lr": 0, "train/grad_norm": 0}
        ...
        # ✅ ДОБАВИТЬ CLEARML ЛОГИРОВАНИЕ ЗДЕСЬ:
        try:
            from clearml import Task
            task = Task.current_task()
        ...
        metrics_dict = {
            ...
            "train/lr": lr,
            "train/grad_norm": grad_norm.detach().item() if torch.is_tensor(grad_norm) else grad_norm
        }
    ...
    def validation_step(self, batch: TensorDict):
        ...
        val_metrics = {"val/loss": loss.item()}
    ...
    def fit(self):
        ...
        # Используем max_steps для контроля длины обучения
        ...
        try:
            for epoch in range(self.config.trainer.total_epochs):
                ...
                for data in tqdm(self.train_dataloader, total=self.steps_per_epoch, 
                            desc=f"Epoch {epoch + 1}/{self.config.trainer.total_epochs}", 
                            disable=rank != 0):
                    
                    # Проверяем ДО обработки батча
                    ...
                    if is_last_step:
                        if rank == 0:
                            print(f"🎉 Training completed! Final validation metrics: {last_valid_metric}")
                            print(f"📊 Total steps: {global_step}")
                        break
                
                # Выход из цикла эпох если достигли max_steps
                if global_step >= max_steps:
                    break
...
def main(config):
    run_sft(config)
    # Force clean interpreter shutdown to prevent lingering background threads from blocking exit
    import sys
    sys.exit(0)
